...
export default function EditTask({ isOpen, taskId, task, onClose, onTaskUpdated }){
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true)
        try {
            const response = await axios.patch(`${url}/task/${taskId}`, data, {
                headers: {
            ...
            toast.error(error.response?.data?.message || error.message || 'Error updating task')
        } finally {
            setLoading(false)
        }
    }
            ...
            <div className="bg-white rounded w-[90%] sm:w-[80%] md:min-w-[300px] max-w-[400px] p-3 sm:p-5 flex flex-col justify-center gap-4 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 min-h-[30px] gap-6">
                        <button type="submit" disabled={loading} className="py-2 text-[13px] bg-blue-600 text-white cursor-pointer rounded hover:bg-blue-700 disabled:bg-gray-400">
